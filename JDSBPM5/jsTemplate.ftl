...
var commonAction = function commonQuery(){
		...
		var lwdw =  Ext.get('${id}lwdw').dom.value;
		var proxyStore = Ext.getCmp("${id}").getStore();
	
		proxyStore.load({params:{start:0,limit:bbar.pageSize,fileTitle:title,startTime:startTime,endTime:endTime,lwdw:lwdw}});
};
...
var newProcess = {
	...
	store.on('beforeload',function(){// 分页条件
		this.baseParams={
			fileTitle:Ext.get('${id}title').dom.value,
			startTime:Ext.get('${id}startTime').dom.value,
			endTime: Ext.get('${id}endTime').dom.value,
			lwdw: Ext.get('${id}lwdw').dom.value
		};
	});
	var ${id}={
	  type:'grid',
