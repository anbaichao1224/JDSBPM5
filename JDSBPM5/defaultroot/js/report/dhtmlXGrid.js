...
$CVSHeader: JDSBPM4/defaultroot/js/report/dhtmlXGrid.js,v 1.2 2011/08/03 09:39:16 liyang Exp $
*/
...
function dhtmlXGridObject(id){
	...
	this.getCellByCnID = function (cnId){

	 for(var jj=1;jj<this.obj.rows.length;jj++){
       var  rrow=this.obj.rows[jj];
   ...
   this.enableEditEvents(true,true,true);
	...
	this.FOOTStart=0;
	this.rowIndex=0;
	this.hasNumberLine=0;
    this.mapCombos=new Hashtable();


	/***************event for def**********************/

	this.onClick4def = function(){
	CellOnClick(this.cell);
	};
	this.onDbClick4def = function(){
    CellDbOnClick(this.cell);
	};


   this.onmouseup4def = function(){
	CellOnMouseUp(this.cell);
	};
	this.onmousedown4def = function(e){

    CellOnMouseDown(this.cell);
	};
	this.onmousemove4def = function(){
    CellOnMouseMove(this.cell);
	};
...
/*MAIN OBJECTS*/
   ...
   this.hdrBox = document.createElement("DIV");
      ...
      this.ev_onHeaderClick=function(headerIndex,event){
            var startTd=  this.obj.rows[0].cells[headerIndex];
          var endTd=this.obj.rows[this.obj.rows.length-1].cells[headerIndex]    ;
         //  alert(startTd+endTd);

          pos.LeftTopCol=startTd.cellIndex;
          pos.LeftTopRow=0;
          pos.RightBottomCol=endTd.cellIndex;
          pos.RightBottomRow= this.obj.rows.length-1;

//            var layoutValue=getLayoutValue();
//            pos.LeftTopCol=startTd.cellIndex;
//            pos.LeftTopRow=layoutValue.colHead.end==-1?0:layoutValue.colHead.end+1-1;
//            pos.RightBottomCol=endTd.cellIndex;
//            pos.RightBottomRow= layoutValue.tabFoot.start==-1?this.obj.rows.length-1:layoutValue.tabFoot.start-1-1;

           this._CreateSelectTd(startTd,endTd);
        return true;
       }
...
/*                         if ((_isMacOS)&&(_isFF))
                     ...
                     if(el.tagName!="TD")
                        el = this.getFirstParentOfType(el,"TD")
                           ...
                           if(el.tagName=="TD" && el.style.cursor!="default"){
                              ...
                              //document.body.onmouseup = new Function("","document.getElementById('"+this.entBox.id+"').grid.stopColResize('"+el.id+"')");
                              var id=this.entBox.id;
                              document.body.onmouseup = function(){document.getElementById(id).grid.stopColResize(el);}
                           }
                        }
      /**
      *   @desc: part of column resize routine
      ...
      *   @topic: 3
      */
      this.stopColResize = function(cell){
                           this.entBox.onmousemove = "";//removeEventListener("mousemove")//
                           document.body.onmouseup = "";
                           this.setSizes();
                           this.doOnScroll(0,1);
						   this.callEvent("onResizeEnd",[this,cell]);
                        }

this.ev_onResizeEnd=function(obj,cell){
  if(colWidthChangeHandler){
      colWidthChangeHandler(obj,cell);
  }
}
      ...
      */
      this.doColResize = function(ev,el,startW,x,tabW){
        if(!this._initWidth){
          this._initWidth=parseInt(this.hdr.parentNode.parentNode.parentNode.parentNode.parentNode.offsetWidth);
        }
        var rootTab=document.getElementById("viewMode");
        if(rootTab){
          var w=rootTab.offsetWidth;
          this._initWidth=Math.min(w,this._initWidth);
        }
                        el.style.cursor = "E-resize";
                        this.resized = el;
                       ...
                       var o=this.hdr.parentNode.parentNode.parentNode.parentNode.parentNode;
                       var tmpW=parseInt(this.hdr.offsetWidth);
                       tmpW+=15;
                       while(o.tagName!="BODY"&&o.getAttribute("resetroot")!="true"){
                         if(tmpW<this._initWidth){
                           o.style.width=this._initWidth;
                         }else{
                           o.style.width=tmpW;
                         }
                           o=o.parentNode;
      ...
      */
       ...
       /**
             ...
             *   @topic: 3
             */
             this.changeRowCursorState = function (ev){
                                  var el = ev.target||ev.srcElement;
                            if(el.tagName!="TD")
                                  el = this.getFirstParentOfType(el,"TD")
                                if(el.className=='INDEX' &&((el.offsetHeight - ev.offsetY)<10)){                                     
             ...
             *   @topic: 3
             */
             this.startRowResize = function(ev){
                                  this.resized = null;
                                  var el = ev.target||ev.srcElement;
                            if(el.tagName!="TD")
                               this.el = this.getFirstParentOfType(el,"TD")
							var y = ev.clientY;
                                  var startW = parseInt(el.offsetHeight)
                                  if(el.tagName=="TD" && el.style.cursor!="default"){
									   if (!el.grid)
									   {
										   el.grid=windowf;
									   } 
                                       el.onmousemove = function(e){this.grid.doRowResize(e||window.event,el,startW,y)}
									   el.onmouseout = new Function("","this.grid.stopRowResize(this)");
                                       el.onmouseup = new Function("","this.grid.stopRowResize(this)");
                                       return false;
                                  } else{
             ...
             *   @topic: 3
             */
           this.stopRowResize = function(el){
           
                                  el.onmousemove = "";//removeEventListener("mousemove")//
                                  document.body.onmouseup = "";
                                   el.onmouseup = "";
                                  this.setSizes();
              ...
              *   @topic: 3
              */
              this.doRowResize = function(ev,el,startW,y){
                  	document.body.style.MozUserSelect = "none";
			        document.body.onselectstart = function() {return false};
                               el.style.cursor = "N-resize";
                                var fcolW = startW + (ev.clientY-y);
					
                                 //var test=window.setTimeout(function(){  el.style.height= fcolW+15; },100);
                              el.style.height= fcolW+15;
							
							   var test=window.setTimeout(function(){ el.grid.setSizes(true) },100);
                                //el.grid.objBox.style.height = el.grid.objBox.style.height + (ev.clientY-y+15);
	  ...
	  *    @param:
	   - zero based index of header row ( used in multirow headers, top row by default )
      *   @type: public
      *   @topic: 7
      */
      ...
      */
        this._doContClick=function(ev){
           //s alert('_doContClick');
                 var el = this.getFirstParentOfType(_isIE?ev.srcElement:ev.target,"TD");
                              
                              this.doClick(el);
                             this.enableContextMenu(cellMenu);




                            if ((!el)||(el.parentNode.idd===undefined)){
                              if (  ev.srcElement==this._selectionObj){
                                    this.enableContextMenu(blockMenu);
                                   el = this.getFirstParentOfType(_isIE?ev.srcElement:ev.target,"DIV");
                               }  else{
                                   this.ev_onHeaderClick(el._cellIndex);
                                   this.enableContextMenu(colMenu);
                              }
                         } else if(  el._cellType=='INDEX'){
                                    var tmpRowNumStr=el.id.replace(/^.+?(\d+).*$/,"$1");
                                    var tmpIdx=parseInt(tmpRowNumStr);
                                    this.selectRow(tmpIdx-1);
                                    this.enableContextMenu(rowMenu);
                            } 
...
//						  if (!r.cellscopy[cInd])
//						  {
//							 for(var j=cInd;r.cellscopy.length>j;j++){
//							  if (r.cellscopy[j])
//							    {
//								  c=r.cellscopy[j];
//
...
//						  }
      ...
      */
      this.editCell = function(){
		...
		editEdCell(this.editor);

                     //initialize editor
                     if(this.editor!=null){
					 	//var rowIdd=this.row.idd;
      ...
      this.editStop = function(){
               ...
               if ((typeof(z)=="string")||(typeof(z)=="number"))
				  ...
				  if (!z) g[g.setImage?"setLabel":"setValue"](g.val);
					 	...
					 	if (g!=null && this.cell.wasChanged){
					 	try{
					updateCell(g);
					}catch (e){
					}
                     ...
                     if (!this.callEvent("onKeyPress",[ev.keyCode,ev.ctrlKey,ev.shiftKey])) return false;
                        ...
                        if(ev.keyCode==13 && !ev.ctrlKey && !ev.shiftKey){
						  ...
						  if(ev.keyCode==13 ){
							...
							//alert("rowInd="+rowInd+"    _cellIndex="+this.cell._cellIndex);
							 this.selectCell(this.rowsAr["r"+(rowInd+1)],this.cell._cellIndex,true);
						  }
                        //TAB
                        if(ev.keyCode==9){
                           this.editStop();
                     ...
                     if (!z) return true;
                        ...
                        if(ev.keyCode==40 || ev.keyCode==38){//&& ev.ctrlKey
                              ...
                              if(ev.keyCode==38 && rowInd!=1){
                                 //if(this.row.rowIndex==0 && this.dynScroll && this.dynScroll!='false')
                                 //   this.doDynScroll("up")
                                 // alert(this.obj._rows(rowInd-2).innerHTML);
                     ...
                     var ed = this.cells4(c);
                        ...
                        if (this._dload && (!this.rowsCol[new_ind])){
                        	this._askRealRows(new_ind,function(){
                        			try{ self.selectCell(new_ind,this.cell._cellIndex,true); } catch(e){};
                        		});
	                        }
                        else{
                           ...
                           if (_isIE) ev.returnValue=false; else ev.preventDefault();
                         ...
                         if(ev.keyCode==37 ){
					       ...
					       this.selectCell(this.row,this.cell._cellIndex-1,true,true);
						   }
						   if(ev.keyCode==39 ){

					       this.selectCell(this.row,this.cell._cellIndex+1,true);
						   }
...
/*
   ...
   */
     this.cells4 = function(cell){
		...
		if (cell.getAttribute("_cellType")==null){
		if (this.cellType[cell._cellIndex]==null){
				return eval("new eXcell_ro(cell)");
		}
   ...
   */
   this.getCombo = function(col_ind){
         ...
         this.mapCombos.put(cell.id,this.combos[col_ind]);
   ...
   this.clearSelection = function(){
                     ...
                     for(var i=0;i<this.selectedRows.length;i++){
                        ...
                        r.className=r.className.replace(/rowselected/g,"");
                            // alert(this.rowsAr[this.selectedRows[i].idd].className);
                            }
                     }
...
/*INTERNAL EVENT HANDLERS*/
   ...
   this.obj.onclick = new Function("e","this.grid._doClick(e||window.event); this.grid.onClick4def(e||window.event); (e||event).cancelBubble=true; ");
   ...
   this.obj.onmousemove= new Function("e","this.grid.changeRowCursorState(e||window.event);this.grid.onmousemove4def(e||window.event);");
   this.obj.onmouseup = new Function("e","this.grid.onmouseup4def(e||window.event);");
   ...
   this.obj.ondblclick = new Function("e","if(!this.grid.wasDblClicked(e||window.event)){return false}; this.grid.onDbClick4def(e||window.event); if (this.grid._dclE) this.grid.editCell(e||window.event);  (e||event).cancelBubble=true;");
...
*/
   dhtmlXGridObject.prototype.enableContextMenu=function(menu){
      //add by wenzhang
       if (this._ctmndx   && this._ctmndx!=menu){
         this._ctmndx.menu.hideBar() ; 
       }
       //
        this._ctmndx=menu;
    };
...
*/
         ...
         if (this._c_order) strAr=this._swapColumns(strAr);
            for(var j=0;j<cellsCol.length;j++){
                 ...
                 if (css1) r.childNodes[j].style.textCSS+=css1;
            }
...
dhtmlXGridObject.prototype.forEachRow=function(custom_code)
{
   for (a in this.rowsAr)
      if (this.rowsAr[a]) custom_code.apply(this,[this.rowsAr[a].idd]);
}
/**
...
/**
*   @desc: prevent horizontal scrolling ( warning - this function change size of grid container )
...
function CellOnMouseUp(cell){

   }


 function CellOnMouseDown(cell){

   }


 function CellOnMouseMove(cell){
