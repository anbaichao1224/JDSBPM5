...
function _formatNumber(num,dotNum,seqFlag){
  ...
  if(seqFlag){
    ...
    left=tmp.substring(0,tmp.length-1);
  }
  if(dotNum<=0){
    return left;
  }else{
...
function _unFormatNumber(val){
  ...
  if(val.substring(val.length-1)=="%"){
    val=parseFloat(val.substring(0,val.length-1))/100;
  }
...
function eXcell_float(cell){
	...
	this.setValue = function(val){
    this._setValue(_formatNumber(val,2,true));
	}
}
eXcell_float.prototype = new eXcell_edAll;
...
eXcell_number.prototype = new eXcell_float;
...
eXcell_rowHeader.prototype = new eXcell_edAll;
...
eXcell_colHeader.prototype = new eXcell_edAll;
