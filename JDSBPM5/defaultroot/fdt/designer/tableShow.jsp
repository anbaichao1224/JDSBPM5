...
<html>
	<head>
		...
		<script type="text/javascript"
			src="<%=contextpath%>js/ext/locale/ext-lang-zh_CN.js"></script>
...
function getTree(currel) {
  var tree = new Tree.TreePanel({
    ...
    animate:true,
    enableDD:false,
    loader: new Tree.TreeLoader({dataUrl:"expression.jsp?expression=$keyIndexTree&crrentIndex=1&txtSearch=$&esbKeyList=FDTFORM"}),
    lines: true,
  ...
  tree.on('beforeload', function(node) {
    ...
    var url = '/expression.jsp?expression=$keyIndexTree&tableName=' + name + "&txtSearch="+(node.attributes.id || "$")+"&type=" + (node.attributes.type || "")+'&esbKeyList=FDTFORM&flowType='+flowType+"&crrentIndex=" + new String(node.attributes.id.length);
  ...
  tree.on('click', function(node, e) {
    ...
    if (node.attributes.tableName) {
        e.stopEvent();
      loadColData(node.attributes.tableName);
     
      tableWin.nodeId=node.attributes.id;
      tableWin.table = node.attributes.tableName;
      panel.setTitle("列信息");
    } else {
      var name = node.id;
      name = name.replace(/^type_/, "");
      ...
      tableWin.nodeId = node.attributes.id;
      loadTableData(node.attributes.tableNames);
 ...
 */
function loadTableData(tableNames,mapdaoid) {
  var url = "/expression.jsp?expression=$tabGridJsonByNames&tableNames=" + tableNames ;
 ...
 * @param type　为table时，显示表名和其中文名,为col时,显示其列的信息
 */
function loadColData(tableName) {
  var url = "/expression.jsp?expression=$getColGridJson&tableName=" + tableName;
