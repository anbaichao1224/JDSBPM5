...
String attid =new UUID().toString();
 ...
 <script>
	...
	var personid = '<ww:property value="$R('personId')"/>';
...
function refFileGridById(){
  ...
  if(obj){
	...
	var accachGridCfg={
		...
		renderTo:getActiveFormId(),	
			 ...
			 
			 {
	                   ...
	                   handler: function(){
	                   	...
	                   	var dia=new Ext.ux.UploadDialog.Dialog({upload_autostart:true,url:contextfile+'fdtattachUpload.action',base_params:{'activityInstId':activityInstId,'formId':formid,'personId':personid}});
	                     dia.on('uploadstart',function(ee,name,c){return false});
	  ...
	  dataUrl:"fdtfileList.action?activityInstId="+activityInstId+"&formId="+formid+"&activityInstHistoryId="+activityInstHistoryId+"&personId="+personid
	};
  ...
  function deleteFile(grid){
	...
	var delAllList=new Array();
	  ...
	  JDS.ajax.Connection.LoadJsonFromUrl(contextfile+'fdtfiledel.action',null,str);
