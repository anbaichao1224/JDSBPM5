...
Ext.onReady(function(){
    ...
    var store = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({   
             url: "/usm/lookOverPersonRoleInfo.do?roleid="+roleid   //从此页获取给人员添加角色的数据
        }),
    ...
    var store2 = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({   
             url: '/usm/getRoleNotPersonInfo.do'  //从此页获取选择人员数据
        }),
        reader: new Ext.data.JsonReader({
            root: 'data',
            totalProperty: 'totalCount',
             ...
             },'-',
            {
		            ...
		            handler:function(){
			            ...
			            for(var i=0,len=row.length;i<len;i++){
			            	...
			            	Ext.Ajax.request({ 
							method:'get', 
							url:"/usm/rolePersonSaveSort.do?txtCheckValue="+txtCheckValue, 
 ...
 buttons: [{
  ...
  //查看所有角色人员
   function showPersonRole(){
   			window.top.createPersonRoleWindow(roleid);
	         store.reload();
        
    }
      
    function showDeleteMessage(){
        Ext.MessageBox.confirm('消息', '确认从当前角色中删除此人员吗？',doDel);
    }
            ...
            if(jsonData.length == 0 ){
                 Ext.MessageBox.alert('消息', '请选择要解除的人员数据。');
		             ...
		             Ext.Ajax.request({
		                //请求地址
		                method: 'POST',      
		                url: '/usm/personRoleDel.do?roleidjsonData=' + jsonData,
		                //成功时回调      
		                success: function(response, options){
		                   Ext.MessageBox.alert('消息', '删除成功');	                   
                           store.reload();
                           window.location="getRoleNameDetailInfo.do?roleid="+roleid; 
		                },
		                //成功时回调      
    ...
    function addPerson()
    {
          Ext.MessageBox.confirm('添加职务', '确认将此人员添加到 [ '+cnname+' ] 角色里吗？',doaddrole);
    }
    function doaddrole(btn)
    {
         if(btn=='yes'){
