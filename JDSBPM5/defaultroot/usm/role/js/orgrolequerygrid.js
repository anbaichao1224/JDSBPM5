...
Ext.onReady(function(){
    ...
    var store = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({   
             url: "/usm/lookOverOrgRoleInfo.do?roleid="+roleid   //从此页获取给岗位添加角色的数据
        }), 
    ...
    var cm = new Ext.grid.ColumnModel([sm, new Ext.grid.RowNumberer(
      ...
      //配置视图信息
      var viewdepart=new Ext.grid.GridView({forceFit:true,sortAscText :'正序', sortDescText :'倒序'});
      viewdepart.columnsText='列显示/隐藏';
 ...
 buttons: [{
   ...
   function showPersonPosition(){
		window.top.createOrgRoleWindow(roleid);
    }
      
    function showDeleteMessage(){
        Ext.MessageBox.confirm('消息', '确认解除此人员所在的岗位吗？',doDel);
    }
            ...
            if(jsonData.length == 0 ){
                 Ext.MessageBox.alert('消息', '请选择要解除的人员数据。');
		             ...
		             Ext.Ajax.request({
		                //请求地址
		                method: 'POST',      
		                url: '/usm/orgRoleDel.do?roleidjsonData=' + jsonData,
		                //成功时回调      
		                success: function(response, options){
		                   Ext.MessageBox.alert('消息', '删除成功');	                   
                           store.reload();
                           window.location="getRoleOrgNameDetailInfo.do?roleid="+roleid; 
		                },
		                //成功时回调      
    ...
    {
          Ext.MessageBox.confirm('添加部门', '确认将此部门添加到 [ '+cnname+' ] 角色里吗？',doaddpostion);
    }
    function doaddpostion(btn)
    {
        if(btn=='yes'){
            ...
            if(jsonData.length == 0 ){
                 Ext.MessageBox.alert('消息', '请在以下人员中选择您要添加的角色部门人员 。');
            }else{
		             Ext.Ajax.request({
		                //请求地址
		                method: 'POST',      
		                url: '/usm/addOrgRoleSave.do?roleid='+roleid+'&roleidjsonData=' + jsonData,
		                //成功时回调      
		                success: function(response, options){
