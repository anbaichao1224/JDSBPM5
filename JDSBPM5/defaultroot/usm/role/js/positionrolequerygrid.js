...
Ext.onReady(function(){
    ...
    var store = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({   
             url: "/usm/lookOverPositionRoleInfo.do?roleid="+roleid   //从此页获取给岗位添加角色的数据
        }),
 ...
 buttons: [{
   ...
   function showPersonPosition(){
		window.top.createPositionRoleWindow(roleid);
    }
            ...
            if(jsonData.length == 0 ){
                 Ext.MessageBox.alert('消息', '请选择要解除的人员数据。');
		             ...
		             Ext.Ajax.request({
		                //请求地址
		                method: 'POST',      
		                url: '/usm/positionRoleDel.do?roleidjsonData=' + jsonData,
		                //成功时回调      
		                success: function(response, options){
		                   Ext.MessageBox.alert('消息', '删除成功');	                   
                           store.reload();
                           window.location="getRolePositionNameDetailInfo.do?roleid="+roleid; 
		                },
		                //成功时回调      
    ...
    {
          Ext.MessageBox.confirm('添加职务', '确认将此岗位添加到 [ '+cnname+' ] 角色里吗？',doaddpostion);
    }
