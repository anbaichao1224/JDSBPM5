...
Ext.onReady(function(){
    ...
    var store = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({   
             url: "/usm/lookOverPersonPositionInfo.do?positionid="+positionid   //从此页获取给人员添加岗位的数据
        }),
    ...
    var store2 = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({   
             url: '/usm/getPositionNotPersonInfo.do'  //从此页获取岗位下没有选择的人员
        }),
        reader: new Ext.data.JsonReader({
            root: 'data',
            totalProperty: 'totalCount',
    ...
    var cm = new Ext.grid.ColumnModel([sm,
     ...
     cm2.defaultSortable = true;
      //配置视图信息
     var viewposition=new Ext.grid.GridView({forceFit:true,sortAscText :'正序', sortDescText :'倒序'});
     viewposition.columnsText='列显示/隐藏';
 ...
 //查看所有岗位人员
   function showPersonPosition(){
			...
			window.top.createPersonPositionWindow(positionid,positionname);
	         store.reload();
   ...
   // Ext.MessageBox.confirm('添加岗位', '确认将此人员添加到 [ '+positionname+' ] 岗位里吗？',doaddpostion);
    }
            ...
            if(jsonData.length == 0 ){
                 Ext.MessageBox.alert('消息', '请选择要解除的人员数据。');
		             ...
		             Ext.Ajax.request({
		                //请求地址
		                method: 'POST',      
		                url: '/usm/personPositionDel.do?positionJsonData=' + jsonData,
		                //成功时回调      
		                success: function(response, options){
		                   Ext.MessageBox.alert('消息', '删除成功');	                   
                           store.reload();
                           window.location="getPositionNameDetailInfo.do?positionid="+positionid; 
		                },
		                //成功时回调      
