	...
	//创建 Store
		var store = new Ext.data.Store( {
			proxy :new Ext.data.HttpProxy( {
				url :'/secure/personsecurejson.do?sysid=' + sysid + '' //从此地址 获取数据
				}),
			...
			tbar : [ {
				...
				handler : function() {
					if (store.totalLength >= 1) {
						Ext.MessageBox.alert('提示', '安全监控人员必须唯一');
					} else {
						showAddPanel();
						eventID = "add";
					}
		...
		var loadFormData = function(grid) {
			...
			if (!_record) {
				Ext.MessageBox.alert('提示', '请选择安全监控管理员');
			} else {
		...
		function doDel(btn) {
			if (btn == 'yes') {
				...
				if (jsonData.length == 0) {
					...
					Ext.Ajax.request( {
						method :'get',
						url :"/usm/personsecureRemove.do?txtCheckValue="
								+ jsonData,
