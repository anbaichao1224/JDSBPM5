...
Ext.onReady(function(){
        ...
        items:[
            ...
            new Ext.tree.TreePanel({
                ...
                loader: new Ext.tree.TreeLoader({
								url: '/usm/dutytreeJson.do?sysid='+sysid+''
		}),
                 rootVisible:true,
                lines:true,
                autoScroll:true,
                root: new Ext.tree.AsyncTreeNode({
                        ...
                        new Ext.Panel({
                            region:'center',
                            id:'topic-grid',
                            html:'<iframe id="views" frameborder="0" src="duty/dutyqueryListgrid.jsp?sysid='+sysid+'&type='+type+'" height=100% width=100%></iframe>'
   ...
   tree.on('click', function(node){
	   if(node.id=='root'){
	        nodeid=node.id;
	   		tree_refresh();
	   		Ext.get('views').dom.src= 'duty/dutyqueryListgrid.jsp?sysid='+sysid+'&type='+type+'';
	   }else{
		 if(type=="manager"){ 
	        Ext.get('views').dom.src= '/usm/dutyInfo.do?sysid='+sysid+'&type='+type+'&dutyid='+node.id;
		  }else{
		  	Ext.get('views').dom.src='/usm/rightTemplate.do?template=duty&sysid='+sysid+'&nodeid='+node.id;
		  }
      }	
