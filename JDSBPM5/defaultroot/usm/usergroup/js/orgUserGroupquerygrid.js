...
Ext.onReady(function(){
    ...
    var store = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({   
             url: "/usm/lookOverOrgUserGroupInfo.do?grpid="+grpid   //从此页获取给部门添加用户组的数据
        }),
 ...
 buttons: [{
   ...
   function showPersonPosition(){
	   window.top.createOrgUserGroupWindow(grpid);
    }
      
    function showDeleteMessage(){
        Ext.MessageBox.confirm('消息', '确认解除此部门所在的用户组吗？',doDel);
    }
            ...
            if(jsonData.length == 0 ){
                 Ext.MessageBox.alert('消息', '请选择要解除的人员数据。');
		             ...
		             Ext.Ajax.request({
		                //请求地址
		                method: 'POST',      
		                url: '/usm/orgUserGroupDel.do?grpidjsonData=' + jsonData,
		                //成功时回调      
		                success: function(response, options){
		                   Ext.MessageBox.alert('消息', '删除成功');	                   
                           store.reload();
                           window.location="getUserGroupNameDetailInfo.do?grpid="+grpid; 
		                },
		                //成功时回调      
    ...
    {
          Ext.MessageBox.confirm('添加职务', '确认将此部门添加到 [ '+grpname+' ] 用户组里吗？',doaddpostion);
    }
