...
Ext.onReady(function(){
    ...
    var store = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({   
             url: "/usm/lookOverPersonUserGroupInfo.do?grpid="+grpid   //从此页获取给人员添加角色的数据
        }),
    ...
    var store2 = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({   
             url: '/usm/getGrpuNotPersonInfo.do'  //从此页获取选择人员数据
        }),
        reader: new Ext.data.JsonReader({
            root: 'data',
            totalProperty: 'totalCount',
    ...
    var cm = new Ext.grid.ColumnModel([sm,new Ext.grid.RowNumberer(
      ...
      var viewusergroup=new Ext.grid.GridView({forceFit:true,sortAscText :'正序', sortDescText :'倒序'});
      viewusergroup.columnsText='列显示/隐藏';
...
width:764,
 ...
 buttons: [{
   ...
   //查看所有用户组人员
   function showPersonUserGroup(){
			window.top.createPersonUserGroupWindow(grpid);
	         store.reload();
    }
      
    function showDeleteMessage(){
        Ext.MessageBox.confirm('消息', '确认解除此人员所在的用户组吗？',doDel);
    }
            ...
            if(jsonData.length == 0 ){
                 Ext.MessageBox.alert('消息', '请选择要解除的人员数据。');
		             ...
		             Ext.Ajax.request({
		                //请求地址
		                method: 'POST',      
		                url: '/usm/personUserGroupDel.do?grpidjsonData=' + jsonData,
		                //成功时回调      
		                success: function(response, options){
		                   Ext.MessageBox.alert('消息', '删除成功');	                   
                           store.reload();
                           window.location="getpersonUserGroupNameDetailInfo.do?grpid="+grpid; 
		                },
		                //成功时回调      
    ...
    {
          Ext.MessageBox.confirm('添加职务', '确认将此人员添加到 [ '+grpname+' ] 用户组里吗？',doaddUserGroup);
    }
    function doaddUserGroup(btn)
    {
