...
.x-grid3-row td,.x-grid3-summary-row td {
	...
	var proxy = new Ext.data.HttpProxy({
		url : '/Chaxun_HqFwWs.action?username='+username+'&password='+password+'&deptid='+deptid+'&flag='+flag
	});
	...
	//退回
	function th(danweiuuid){
      	Ext.Ajax.request({
			url : '/HqYewuAction_updateFlag.action',
			params : {
				uuid : danweiuuid
			},
  ...
  function hqqs(uuid){
			...
			var updatewin = new Ext.Window({
					...
					height : _height,
					minHeight : 200,
					html:"<iframe id='djiframe1' name='djiframe1' width='" + (_width-30)  + "' height='" + (_height - 60) + "' src='HqYewuAction_hqqs.action?uuid="+uuid+"'></iframe>",
					tbar:[
						{
							id : 'djsave',
							...
							cls : "x-btn-text-icon",
							handler : function(){
							   var fn = Ext.get('djiframe1').dom.contentWindow.document.getElementById("qs");
                                   fn.submit();
                                   alert('签收成功');
								store.reload();
								updatewin.close();
