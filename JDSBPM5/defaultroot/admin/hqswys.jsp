...
.x-grid3-row td,.x-grid3-summary-row td {
	...
	var proxy = new Ext.data.HttpProxy({
		url : '/Chaxun_HqSwYs.action?username='+username+'&password='+password+'&deptid='+deptid+"&flag="+flag
	});
	...
	//取回已经交办理的
	function swquhui(uuid){
      	Ext.Ajax.request({
			url : '/HqYewuAction_swupdateFlagTo0.action',
			params : {
				uuid : uuid
			},
		...
		failure : function(resp, opts) {
  		...
  		function showshowwin(uuid){
			...
			var updatewin = new Ext.Window({
					...
					height : _height,
					minHeight : 200,
					html:"<iframe id='djiframe1' name='djiframe1' width='" + (_width-30)  + "' height='" + (_height - 60) + "' src='HqYewuAction_swhqjbl.action?uuid="+uuid+"'></iframe>",
					tbar:[
						{
							id : 'djsave',
							...
							cls : "x-btn-text-icon",
							handler : function(){
							   var fn = Ext.get('djiframe1').dom.contentWindow.document.getElementById("swhqjbl");
                               var faSongRen = Ext.get('djiframe1').dom.contentWindow.document.getElementById("sendrange").value;
