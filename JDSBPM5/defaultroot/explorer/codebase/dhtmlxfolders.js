...
*/
	...
	*/
	function dhtmlxFolders(parent){
		...
		this.cont.onmouseup = function(e){ 
			...
			if (event.button!=2){
					if(clickTime-self.clickTime<500){
       ...
       document.body.onmouseup 	= function(e){ 
		...
		//this.cont.onmouseout = function(e){ return self._onmoutHandler(e||event); };		
	...
	*/
	...
	dhtmlxFolders.prototype.setSizes=function(){
			this.cont.style.height=this.cont.parentNode.scrollHeight-(_isIE?0:6)+"px";
	}	
	/**
	...
	*/
	dhtmlxFolders.prototype._onmupHandler=function(e){	
		...
		if (e.button==2){
			if (id!=null){
				this.selectItem(id);
				var tType=this.getItem(id).data.dataObj.getAttribute("type");
				
				 if (tType == "file" && this.fileContextMenu) {
				 	this._ctmndx=this.fileContextMenu;
				 }else if (tType=="dir" && this.folderContextMenu){
				 	this._ctmndx=this.folderContextMenu;
				 }
			}else if (this.bodyContextMenu){
				this._ctmndx=this.bodyContextMenu;
			}
		}
	...
	*/
	dhtmlxFolders.prototype._onclickHandler=function(e){
       ...
       if (this.itemContextMenu){
			this.itemContextMenu.setContextZone(this.getItem(id),id)
		}
...
//xml-xsl based
	function dhtmlxFoldersXMLBasedGeneric(){
		...
		this.render = function(){
			...
			if (this.data.master._set.editable){
