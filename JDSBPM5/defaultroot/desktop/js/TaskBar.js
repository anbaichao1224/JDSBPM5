...
Ext.extend(Ext.ux.TaskBar, Ext.util.Observable, {
    init : function(){
			...
			height: 350,
			shadow: true,
			//title: 'Todd Murdock', //get_cookie('memberName'),
			width: 350
		...
		this.clockPanel=new Ext.Panel({
			id:'ux-clock-panel',
            html:'<div id="ux-clock-panel"><div id="ux-clock-wrap" ></div><div id="ux-clock-text" class="ux-clock-text"></div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick="loadMsgref()"><img src="/usm/images/laba.png"/></a></div>',			width:300,	
			region:'east',
			autoShow:true
		});
		...
		var panelWrap = new Ext.Container({
			...
			items: [this.quickStartPanel,this.taskButtonPanel,this.clockPanel],
			layout: 'border',
			region: 'center'
		});
	   ...
	   Ext.TaskMgr.start({
			        run: function(){
			            Ext.fly('ux-clock-text').update(new Date().format('g:i:s A'));
			        },
			        interval: 1000
			    });
...
Ext.ux.TaskButtonsPanel = Ext.extend(Ext.BoxComponent, {
	...
	this.setActiveButton(btn);
	    var imgUrl=win.module.thumb?win.module.thumb:Ext.BLANK_IMAGE_URL;
...
Ext.ux.TaskBar.TaskButton = function(win, el){
    ...
    Ext.ux.TaskBar.TaskButton.superclass.constructor.call(this, {
        iconCls: win.iconCls,
        text: Ext.util.Format.ellipsis(win.text?win.text:win.title, 12),
        //tooltip: win.taskbuttonTooltip || win.title,
		xbtnimg:win.module.icon,
        renderTo: el,
        handler : function(){
    ...
    onRender : function(){
		if (this.xbtnimg){
			this.template = new Ext.Template(
                    ...
                    '<td class="x-btn-left"><i>&#160;</i></td><td class="x-btn-center"><img style="width:16px;height:16px" src="'+this.xbtnimg+'"></img><em unselectable="on"><button class="x-btn-text" type="{1}">{0}</button></em></td><td class="x-btn-right"><i>&#160;</i></td>',
                    "</tr></tbody></table>");
...
Ext.ux.QuickStartPanel = Ext.extend(Ext.BoxComponent, {
	...
	add : function(config){
		if (config.index && config.index>-1){
			var li = this.strip.createChild({tag:'li'}, this.items[config.index].getEl().dom.parentNode); // insert before the edge
		}else{
			var li = this.strip.createChild({tag:'li'}, this.edge); // insert before the edge
		}
	 ...
	 quickStartDragZone = new JDS.dd.JDSDragZone(btn.el, {
                ddGroup: 'DesktopShortcuts',
				moduleId:config.moduleId,
                scroll: false
            });
			
	  quickStartItemTarget = new JDS.dd.JDSDropTarget(btn.getEl(), {
	 	    items:this.items,
            ddGroup: 'DesktopShortcuts',
            overClass: 'dd-over',
			moduleId:config.moduleId,
			...
			var quickstart=JDSDesk.getDesktop().config.launchers.quickstart;
			   if (data.moduleId){
				   	 var sIndex=getIndexOfArr(data.moduleId,quickstart);
					 var tIndex=getIndexOfArr(this.moduleId,quickstart);
					 var nIndex=tIndex>sIndex?tIndex-1:tIndex;
					
					if (sIndex>-1){
						 JDSDesk.getDesktop().removeQuickStartButton(data.moduleId, true);
					}
					btn=JDSDesk.getDesktop().addQuickStartButton(data.moduleId,false,nIndex);
					var sIndex=getIndexOfArr(btn,this.items);
					quickstart.splice(sIndex,0,data.moduleId);
			  	}
			 return true;
		    }
        });	
		
		quickStartTarget = new JDS.dd.JDSDropTarget(this.getEl(), {
            ddGroup: 'DesktopShortcuts',
            overClass: 'dd-over',
			notifyDrop: function(dd, e, data) {
   	        if (data.moduleId){
				   	JDSDesk.getDesktop().addQuickStartButton(data.moduleId, true);
			  	}else if(data.selectedItems && data.selectedItems.length>0 ){
					 for(var i = 0, len = data.selectedItems.length; i < len; i++){
						JDSDesk.getDesktop().addQuickStartButton(data.selectedItems[i].moduleId, true);
					}
		...
		if (config.index && config.index>-1){
			this.items.splice(config.index,0,btn);
		}else{
			this.items.push(btn);
		}
		this.delegateUpdates();
