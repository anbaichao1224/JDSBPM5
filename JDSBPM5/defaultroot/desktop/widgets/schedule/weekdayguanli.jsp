...
<html>
  <head>
    ...
    <title>My JSP 'weekday.jsp' starting page</title>
...
<script src="/desktop/widgets/schedule/dhtmlxScheduler/locale_cn2.js" type="text/javascript" charset="utf-8"></script>
...
<script language="JavaScript">
	...
	function schedulerInit(){
		...
		var oday1 = formatDate(new Date());
			...
			scheduler.templates.event_header=function(){
				return '';
			}
			...
			//scheduler.attchEvent("onEventCreated",ceshi);
			...
			scheduler.init('scheduler_here',new Date(parseInt(year), parseInt(month)-1, parseInt(day)), "month");
	...
	function addOrUpdate(event_id,event_object){
		...
		new Ajax.Request('/gzrdyAction.action', {
			 method: 'post',
	...
	function del(event_id,event_object){
		var retvalue = true;
		new Ajax.Request('/gzrdyAction_delete.action', {
			 method: 'post',
	...
	function beforeViewChange(old_mode, old_date, mode , date){
		...
		var url = "/gzrdyAction_find.action?date=" + date1.toLocaleString() + "&queryModel=" + mode1 + "&oday=" + datestr;
		scheduler.load(url);
		return true;  
	}
