 ...
 */
 ...
 Ext.chart.Chart = Ext.extend(Ext.FlashComponent, {
   ...
   setSeriesStylesByIndex:function(index,style){
     var s=Ext.encode(style);
     if(this.swf&&this.swf.setSeriesStylesByIndex){
       this.swf.setSeriesStylesByIndex(index,s);
     }
   },
   /**
     * 设置具体的图形的显隐状态
     ...
     */
    showSeries:function (idx, flag) {
      this.setSeriesStylesByIndex(idx, {visibility:flag ? "visible" : "hidden"});
      if(!this.seriesShowStatus){
        this.seriesShowStatus={};
      }
      this.seriesShowStatus[idx]=flag;
    },
   /**
    * 取具体图形的显示状态,只是保存的设置时的状态,不是取的其真正状态,默认为true
    * @param names
    */
    getSeriesShowStatus:function(idx){
      if(!this.seriesShowStatus){
        this.seriesShowStatus={};
      }
      return this.seriesShowStatus[idx]===false?false:true;
    },
