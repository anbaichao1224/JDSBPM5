		...
		var addForm = new Ext.form.FormPanel({
			...
			items : [ {
			    ...
			    items:[{
			       ...
			       items:[combo,
			       {fieldLabel:'发送人',name:'sendbean.sendor',width:180,value:'<ww:property value="$currPerson.name"/>'},
			       {fieldLabel:'发送时间',xtype:'datefield',emptyText:'请选择',
		              format:'Y-m-d',width:200,name:'sendbean.sendtime', blankText: '时间不能为空',allowBlank: false},
			       {fieldLabel:'签发时间',xtype:'datefield',emptyText:'请选择',
	              	format:'Y-m-d',width:200,name:'sendbean.sendSignTime',blankText: '时间不能为空',allowBlank: false},
			       ...
			       defaultType:'textfield',
			       items:[jjcdcombo,
			         {fieldLabel:'发送单位',width:180,name:'sendbean.senddept',value:'<ww:property value="$currPerson.orgList[0].name"/>'},
			         {fieldLabel:'签发人',width:180,name:'sendbean.sendSigner'},
			         {fieldLabel:'签发单位',width:180,name:'sendbean.sendSigndept'},
			         {fieldLabel:'份数',width:180,name:'sendbean.sendCopy' ,blankText: '只能输入数字',
			             regex:/^[0-9]+$/, 
          			     regexText: '只能输入数字'},
			         {fieldLabel:'联系单位',width:180,name:'sendbean.sendLxdw'}
     ...
     function save(){
        if(Ext.getCmp('addform').form.isValid()) {
			 Ext.Ajax.request({
			 	...
			 	'sendbean.sendtime':Ext.get('sendbean.sendtime').dom.value,
			 	...
			 	'sendbean.sendSigndept':Ext.get('sendbean.sendSigndept').dom.value,
			 	'sendbean.sendCopy':Ext.get('sendbean.sendCopy').dom.value,
			 	'sendbean.sendLxdw':Ext.get('sendbean.sendLxdw').dom.value,
...
<input type="hidden" name="liststatus" id="liststatus" value='<%=request.getParameter("liststatus")%>'/>
  ...
  <tr>
    ...
    <td colspan="3"><input size="70" name="deptnames" type="text" id="deptnames" onkeydown="return false;"  size="50"/>
