...
<html>
	...
	<script type="text/javascript">
			...
			var proxy = new Ext.data.HttpProxy({
				url :'RootAction_showListRoot.action?isRoot=1'
			});
				...
				var grid = new Ext.grid.GridPanel({
					...
					[{
						...
						handler : function(){
							...
							var addwin = new Ext.Window({
									...
									minHeight : 40,
									y:0,
									html:"<iframe id='addiframe' name='addiframe' width='" + (_width-10)  + "' height='" + (_height - 10) + "' src='RootAction_addRoot.action?'></iframe>",
									tbar:[{
											id : 'rjsave',
					...
					handler : updatepage
						...
						handler : function() {
							...
							Ext.Ajax.request({
								url : 'RootAction_deleteRoot.action',
								params : {
									catalogId : deluuid
								},
								success : function(resp, opts) {
									if(resp.responseText=='true'){
										...
										Ext.Msg.alert('信息', '删除单位失败，因为其有部门或是有人员而不允许删除');
									}
		...
		function requery() {
			...
			function showupdatewin(uuid){
				...
				var updatewin = new Ext.Window({
					...
					minHeight : 200,
					y:0,
					html:"<iframe id='updateiframe' name='updateiframe' width='" + (_width-30)  + "' height='" + (_height - 70) + "' src='RootAction_updateRoot.action?uuid="+uuid+"'></iframe>",
					tbar:[{
						id : 'rjsave',
						icon : "/usm/img/save.gif",
						text : '修  改',
