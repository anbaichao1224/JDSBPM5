...
<%
	...
	String catalogUuid = request.getParameter("uuid");
...
<html>
	...
	<script type="text/javascript">
		...
		var proxy = new Ext.data.HttpProxy({
			url :'personAction_findperson.action?orguuid=<%=catalogUuid %>'
		});
		
		//为了让修改方法在标题那里的超链接能调用到
		...
		Ext.onReady(function() {
			...
			var sm = new Ext.grid.CheckboxSelectionModel ({
	            ...
	            //   if(record.get("personid")== '<%=personId%>'){   
         ...
         // 不允许使用点击表格形式修改选择  
			...
			var grid = new Ext.grid.GridPanel({
				...
				['-',{
					...
					handler : function(){
						...
						var addwin = new Ext.Window({
							...
							minHeight : 40,
							y:0,
							html:"<iframe id='addiframe' name='addiframe' width='" + (_width-10)  + "' height='" + (_height - 10) + "' src='personAction_toAddPerson.action?catalogUuid=<%=catalogUuid%>'></iframe>",
							tbar:[{
									id : 'rjsave',
									...
									cls : "x-btn-text-icon",
									handler : function(){
										var fn = Ext.get('addiframe').dom.contentWindow.document.getElementById("addPerson");
										//判断排序序号不能为空
											var catalogOrder = Ext.get('addiframe').dom.contentWindow.document.getElementById("other");
									...
									cls : "x-btn-text-icon",
									handler : function(){
										var fn = Ext.get('addiframe').dom.contentWindow.document.getElementById("addZml");
										fn.reset();
									}
								}
							]
				...
				handler : updatepage
						...
						handler : function() {
							...
							Ext.Ajax.request({
								url : 'tbmAction_deleteZml.action',
								params : {
									catalogId : deluuid
								},
								...
								success : function(resp, opts) {
									...
									if(resp.responseText=='true'){
										...
										Ext.Msg.alert('信息', '删除部分失败，因为其有部门或是人员而不允许删除');
									}
								},
				...
				function queryZL() {
					store.load({
						params : {
							start : 0,
							limit : pgsize,
							biaoti : Ext.get('docname').dom.value,
			...
			function showupdatewin(uuid){
				...
				var updatewin = new Ext.Window({
					...
					minHeight : 200,
					y:0,
					html:"<iframe id='updateiframe' name='updateiframe' width='" + (_width-30)  + "' height='" + (_height - 70) + "' src='tbmAction_toUpdateYm.action?catalogId="+uuid+"'></iframe>",
					tbar:[{
						id : 'rjsave',
						icon : "/usm/img/save.gif",
						text : '修  改',
						cls : "x-btn-text-icon",
						handler : function(){
							var fn = Ext.get('updateiframe').dom.contentWindow.document.getElementById("updateZml");
							//判断排序序号不能为空
