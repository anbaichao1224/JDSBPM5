...
var resetDock = function(id) {
	...
	jQuery(docks[id].e).each(function(i) { 
		...
		strHTML+='<table  style=\"display :inline;margin: 0; color:#ffffff;	font:12px Arial,sans-serif;	text-align: center;border:0;"><tr><td><a style="margin: 0; border:0;" href="'+this.href+'" title="'+this.descripcio+'"><img alt="'+this.title+'" style="padding: '+ptop+'px 0 '+pbottom+'px 0; margin:0; border:0;" width="'+this.costatActual+'" height="'+this.costatActual+'" src="'+((this.costatActual>docks[id].conf.iconMinSide)? this.srcBig  : this.srcSmall)+'" /></a></td></tr><tr><td>'+this.title+'</td><tr>';	
		
		divWidth+=this.costatActual;
	});
	
	
	strHTML="<div class=\"docking\" style=\"margin:0; padding:0; border:0; width: "+(divWidth+500)+"px; height:"+(docks[id].conf.iconMaxSide+10)+"px; \">"+strHTML+"</div>";
...
jQuery.fn.extend({
   addDockEffect: function(conf) {
		...
		jQuery("a",docks[id].dockPare).each(function (i) {	
			...
			docks[id].e[i].title=(aname)? aname : (imgalt)? imgalt : docks[id].e[i].descripcio.substr(6,16)+"..." ;
		});
		startDock(id);
