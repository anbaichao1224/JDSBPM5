...
JDS.bpm.Form.SelectPerFormDataBf = function(cfg) {
	Ext.apply(this, cfg);
    var name = cfg.name;
	...
	this.getTree = function(treeType, routeDefId, checkModel, title) {
		var url = context + 'SelectPerformerJAONBF.action?activityInstId='
				+ this.module.activityInstId+'&activityInstHistoryId='+activityInstHistoryId;
		var Tree = Ext.tree;
...
Ext.extend(JDS.bpm.Form.SelectPerFormDataBf, Ext.util.Observable, {
	...
	render : function() {
		...
		if (this.isNoSelect) {
			...
			this.readTree = this.getTree("READ",
					this.performTrees[0].routeDefId, "multiple", "");
