...
function saveRoll(){
	...
	if(checkForm('addRoll',stime,etime)){
		Ext.Ajax.request({
			...
			success:function(o){
				alert("添加成功");
				if(parent.Ext.getCmp('roll-list')!=null){
					parent.Ext.getCmp('roll-list').getStore().reload();
					parent.Ext.getCmp('addRoll').close();
				}else{
					parent.Ext.getCmp('updateRollInfo').close();
					//parent.Ext.getCmp('roll-columntree').root.reload();
				}
...
<input type="hidden" name="rollbean.createdate" id="uuid" value='<ww:property value="rollbean.createdate"/>'/>
  ...
  <tr>
    ...
    <td width="272">
    <select name="rollbean.miji" id="miji">
    	<option value="明文">明文</option>
    </select><span style="color:red;">*</span></td>
  </tr>
  <script text="text/javascript">
  		if('<ww:property value="rollbean.miji"/>'!=''){
  		document.getElementById('miji').value='<ww:property value="rollbean.miji"/>';
 		}
  </script>
  <tr>
  ...
  <tr>
    <td colspan="4" align="center"><input type="button" name="savematter" value="保存" onclick="saveRoll()" />&nbsp;&nbsp;</td>
  </tr>
