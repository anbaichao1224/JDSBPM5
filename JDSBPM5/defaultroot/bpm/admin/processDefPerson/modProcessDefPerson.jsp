<%
/**
 *    $RCSfile: modProcessDefPerson.jsp,v $
 *    $Author: liyang $
 *    $Revision: 1.2 $
 *    $Date: 2011/08/03 09:39:19 $
 */
%>
...
<%
	try {
        ...
        String querySQL = "SELECT PERSON_SELECT_FORMULA FROM ADMIN_BPMSYS_PERSON WHERE PERSON_ID='"+PERSON_ID+"' AND APPNAME='"+APPNAME+"' AND MODULE_NAME='BPD'";
        int state = dba.execute(querySQL);
        if(state == -1) {
            response.sendRedirect("error.jsp?errorMsg="+URLEncoder.encode("指定的数据库记录不存在。")+"&returnURI=processDefPersonList.jsp");
			dba.close();
            return;
        }
...
<form name="frm" method="post" action="modProcessDefPerson_do.jsp" onSubmit="return _check();">
  ...
  <tr> 
    <td height="500" align="center" valign="top"> <TABLE width="100%" border=0 align=center cellspacing="3">
      <tr height="21" valign="bottom">
        <td width="500" class="lin3"><font color="FB4303">■ 当前位置 》编辑流程定义人员</font></td>
      </tr>
      <tr>
        <td height="8"> </td>
      </tr>
