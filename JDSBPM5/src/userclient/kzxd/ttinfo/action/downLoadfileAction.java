...
//			ServletActionContext.getResponse().setHeader("Content-Disposition","attachment;filename=" + fileName);
		...
		public String execute() throws Exception {
			...
			writeResponse();
//			 发现企图下载不在 /download 下的文件, 就显示sss空内容 
			if(!file.isFile()) 
			{
				fileName="文件错误";
		...
		public void getFileAttribute(){
	        	    ...
	        	    Connection conn = null;
		   	    	...
		   	    	conn = dbbase.getConn();
		   	    	TtInfoDAO  attdao = new TtInfoDAO();
		   	    	attdao.setUuid(uuid);
		   	    	
		   	    	factory = new DAOFactory(conn,attdao);
		   	    	List dbfiles = factory.find();
		   	    	for(int i=0;i<dbfiles.size();i++){  //实际只有一条
		   	    		TtInfoDAO  ba = (TtInfoDAO)dbfiles.get(i);//文件下载目录路径 
		        	   this.downloadDir = rootpath + ba.getTtpath(); 
		        	   this.downloadFile =  rootpath+ba.getTtpath()+ba.getUuid()+".doc";
		   	    	}
		            }catch (Exception e) {
