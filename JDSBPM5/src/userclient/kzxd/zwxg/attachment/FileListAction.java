...
public class FileListAction extends BPMActionBase{
	...
	public List getAllFile()
	{
		 ...
		 Connection conn = null;
			...
			//	{
			    	...
			    	zattdao.setProcessinstid(proceInst.getProcessInstId());
			    	zattdao.setFormid(formId);   //取出当前流程当前表单上的附件
			    	...
			    	if(dbfiles != null && size >0)
			    	{
			    		...
			    		{
			    			ZwxgAttachmentDAO  ba = 	(ZwxgAttachmentDAO)dbfiles.get(i);
			 ...
			 //   	  }
				...
				if(dbfiles == null || dbfiles.size()<1) {
			    		factory = new DAOFactory(conn,zattdao);
				    	zattdao.setActivityinstid(proceInst.getProcessInstId()); 
				    	zattdao.setFormid(formId);
				    	List histryfiles = factory.find();
				    	if(histryfiles != null && histryfiles.size()>0)
				    	{
				    		for(int i=0;i<histryfiles.size();i++)
				    		{
				    			ZwxgAttachmentDAO  ba = 	(ZwxgAttachmentDAO)histryfiles.get(i);
	...
	public void addFileToBeanList() throws PersonNotFoundException
	{
		...
		for(int i=0;i<fileList.size();i++)
		{
			...
			ZwxgAttachmentDAO  ba = 	(ZwxgAttachmentDAO)fileList.get(i);
