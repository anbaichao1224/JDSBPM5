 ...
 * @version 2.0
 */
public class RouteLogAction extends BPMActionBase {
		...
		if (webCurrentWrpList == null) {
			...
			for (int i = 0; activityList.size() > i; i++) {
				...
				CurrentWrp currentWrp = new CurrentWrp(activityInst,
						this.client);
				if(currentWrp.getPerformers().size()!=0){
					if(!activityInst.getState().equals("suspended")){
	...
	public List getHistoryWrpList() throws BPMException {
		if (webHistoryList == null) {
			...
			for (int i = historyList.size()-1; historyList.size()-1 > 0; i--) {
				if(i<0){
					break;
				}
				...
				HistoryWrp historyWrp = new HistoryWrp(activityHistory,
						this.client);
				if(historyWrp.getPerformers().size()!=0){
